...
library(tidyverse)
library(ggplot2)
library(VIM)  # For visualizing missing values if available
...
The heatmap reveals the distribution of missing values in the "ESTIMATE" column, across various Group categories and Time Periods. The blank spaces in the heatmap represent combinations of subgroup and TIME_PERIOD where no data exists in the dataset. This means that for certain subgroups during specific time periods, no rows are present in the data.
